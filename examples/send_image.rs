/*!
Example: Send an image message via WeCom (Kf) send_msg
...
- WXKF_MEDIA_ID must be obtained from the temporary material upload API.
*/

use anyhow::{Context, Result};
use dotenvy::dotenv;
use std::env;
use wxkefu_rs::send_msg::{MediaContent, SendMsgPayload, SendMsgRequest};
...
async fn main() -> Result<()> {
    ...
    let media_id =
        env::var("WXKF_MEDIA_ID").context("set WXKF_MEDIA_ID (image media_id from upload API)")?;
    ...
    // 2) Build the send_msg request for an image
    let req = SendMsgRequest {
        touser,
        open_kfid,
        msgid,
        payload: SendMsgPayload::Image {
            image: MediaContent { media_id },
        },
    };
