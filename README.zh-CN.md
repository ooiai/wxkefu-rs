...
Redis 缓存 access_token（并发安全自动刷新）

简介

- 目标：通过 Redis 缓存 access_token，降低 gettoken 调用频率；在到期前提前刷新；在多实例并发下用分布式锁防止“惊群”。
- 适用：企业微信（微信客服 Kf）access_token；公众号/小程序也可参考，但 Kf 接口只接受 WeCom token。

核心策略

- 安全缓存：value 存 JSON（access_token + expires_at），设置 Redis TTL；注意不在日志打印明文 token。
- 刷新提前（refresh-ahead）：当剩余有效期小于阈值（默认 300s），后台尝试刷新（非阻塞）。
- 分布式锁：SET NX EX 锁 30s，只有持锁者刷新，其余节点短暂等待后重试，避免并发下频繁打到上游。
- 安全边界（safety margin）：从官方 expires_in（通常 7200s）中减去 120s，避免边界时刻出错。
- 并发等待：若锁已被其他实例持有，任务以短抖动轮询，等待最多 5s；若仍未可用，返回并提示重试。

环境变量

- REDIS_URL（默认 redis://127.0.0.1/）
- WXKF_CORP_ID（以 ww 开头）
- WXKF_APP_SECRET（微信客服后台 → 开发配置中的 Secret）

示例代码

```wxkefu-rs/examples/get_token_redis.rs#L1-115
use dotenvy::dotenv;
...
- `tm.get_access_token()`：优先返回缓存；过期或缺失时持锁刷新；临近过期触发后台刷新。
- `tm.force_refresh()`：强制刷新（持锁），请谨慎使用。
- `tm.invalidate()`：显式失效缓存；配合 40001/40014 等错误进行降级重试。

生产建议

- 统一 namespace（如 `wxkefu:token`），避免与其他业务键冲突。
- 预留存储空间 ≥ 512 字节以容纳 token。
- 监控刷新失败与锁等待时延；合理设置 `refresh_ahead/lock_ttl/max_wait`。
- 处理早失效：若业务 API 返回 40014/40001，请先 `invalidate + force_refresh`，再重试一次。
